...
function formatIssueBody(data: z.infer<typeof bugReportSchema>): string {
  ...
  if (data.includeEnvironmentInfo && data.environmentInfo) {
    ...
    if (data.environmentInfo.boltVersion) {
      body += `- kuberha.ai: ${data.environmentInfo.boltVersion}\n`;
    }
  ...
  body += '---\n*Submitted via kuberha.ai bug report feature*';
...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  try {
    ...
    const targetRepo =
      (context?.cloudflare?.env as any)?.BUG_REPORT_REPO || process.env.BUG_REPORT_REPO || 'kuberha/kuberha.ai';
    ...
    const octokit = new Octokit({
      auth: githubToken,
      userAgent: 'kuberha.ai-bug-reporter',
    });
